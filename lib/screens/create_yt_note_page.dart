...
class _CreateYTNotePageState extends State<CreateYTNotePage> {
          ...
          if (_result == null) ...[
            ...
            if (!_isDownloading) ...[
              ElevatedButton.icon(
                ...
                onPressed: (_isGenerating || _isDownloading) ? null : _selectYoutubeFile,
              ),
            ] else ...[
  ...
  Future<String?> downloadYouTubeCaption() async {
    ...
    try {
      ...
      // Get the first available caption track (you can modify this to select a specific language)
      final track = manifest.tracks.first;

      // Download the caption
      final caption = await yt.videos.closedCaptions.get(track);

      // Convert caption to plain text
      String captionText = '';
      for (final caption in caption.captions) {
      ...
      print('Error downloading caption: $e');
      return null;
    }
  }
  ...
  Future<void> _generateNote() async {
    ...
    try {
      ...
      final captionText = await downloadYouTubeCaption();
